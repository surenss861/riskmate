...
const ROUTE = '/api/analytics/compliance-rate'
  ...
  try {
    ...
    if (!isActive || !hasAnalytics) {
      return NextResponse.json(
        { period: periodLabel, signatures: 0, photos: 0, checklists: 0, overall: 0, locked: true },
        { status: 200, headers: { 'X-Request-ID': requestId } }
      )
    }

    return proxyToBackend(request, '/api/analytics/compliance-rate', { method: 'GET' })
  } catch (error: unknown) {
    const err = error as { message?: string }
    console.error('Analytics compliance-rate proxy error:', error)
    const rid = getRequestId(request)
    const { response, errorId } = createErrorResponse(
      'Failed to fetch compliance rate',
      'PROXY_ERROR',
      {
        requestId: rid,
        statusCode: 500,
