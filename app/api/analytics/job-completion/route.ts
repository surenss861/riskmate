...
const ROUTE = '/api/analytics/job-completion'
...
export async function GET(request: NextRequest) {
  try {
    ...
    return proxyToBackend(request, '/api/analytics/job-completion', { method: 'GET' })
  } catch (error: unknown) {
    const err = error as { message?: string }
    console.error('Analytics job-completion proxy error:', error)
    const rid = getRequestId(request)
    const { response, errorId } = createErrorResponse(
      'Failed to fetch job completion',
      'PROXY_ERROR',
      {
        requestId: rid,
        statusCode: 500,
