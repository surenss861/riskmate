...
const ROUTE = '/api/analytics/status-by-period'
...
export async function GET(request: NextRequest) {
  try {
    ...
    return proxyToBackend(request, '/api/analytics/status-by-period', { method: 'GET' })
  } catch (error: unknown) {
    const err = error as { message?: string }
    console.error('Analytics status-by-period proxy error:', error)
    const rid = getRequestId(request)
    const { response, errorId } = createErrorResponse(
      'Failed to fetch status by period',
      'PROXY_ERROR',
      {
        requestId: rid,
        statusCode: 500,
