 ...
 * This route is only called by direct API consumers (e.g. integrations calling the Next.js API). The web client uses
 * BACKEND_URL (Express) via lib/api.ts jobsApi.create() and never hits this route, so no deduplication guard is needed.
 * Webhook emission here is for those direct callers; do not proxy web-client job creation through both Express and this route or webhooks may be doubled.
 */
