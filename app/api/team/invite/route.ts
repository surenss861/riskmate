...
export async function POST(request: NextRequest) {
  try {
    ...
    // Webhook team.member_added: emitted from this route when invite is created via Next.js. The Express invite route (apps/backend/src/routes/team.ts) also emits for invites via that path. Each request must be handled by one stack only — do not proxy invite creation to the other stack or duplicate webhook deliveries will occur. Enqueue failures are not retried; monitor [WebhookTrigger] Fetch endpoints failed.
