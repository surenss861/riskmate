...
function addHeaderFooter(
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    if (pageIndex === 1 && page2ColumnLayout) { // Page 2 (0-indexed, so page 2 is index 1)
      ...
      currentY = writeLine(sanitizeText(orgLine), 8, STYLES.fonts.body, 14, { noWrap: true }) // Atomic line, proper spacing
      ...
      // Source tables summary - CRITICAL: sanitize and use writeLine for atomic separation
      const sourcesText = sanitizeText('Sources: jobs, incidents, attestations')
      currentY = writeLine(sourcesText, 8, STYLES.fonts.body, 11) // Use writeLine for atomic line
      ...
      if (metadataHash) {
        ...
        // Add raw hash line for copy/paste and system verification
        // CRITICAL: Make "Metadata hash (raw):" and raw hash lines fully atomic in extraction
        // Treat label as its own atomic line, then render raw hash on deterministic next lines
        // This prevents text extraction collisions where labels and hash interleave
        doc.moveDown(0.2)
        const rawHashLabel = sanitizeAscii('Metadata hash (raw):')
        // CRITICAL: Label must be its own atomic line with proper spacing
        currentY = writeLine(rawHashLabel, 7, 'Courier', 11, { noWrap: true }) // Atomic label line
        ...
        const hashChars = metadataHash.split('')
        const charsPerLine = 22
        const hashLines: string[] = []
        for (let i = 0; i < hashChars.length; i += charsPerLine) {
          hashLines.push(hashChars.slice(i, i + charsPerLine).join(''))
        }
        
        // Render each hash line as atomic (noWrap to prevent mid-hash breaks)
        // CRITICAL: Each line is a separate atomic writeLine call to prevent interleaving
        for (const hashLine of hashLines) {
          currentY = writeLine(sanitizeAscii(hashLine), 7, 'Courier', 9, { noWrap: true })
        }
        ...
        currentY = writeLine(pdfHashNote, 7, STYLES.fonts.body, 11, { noWrap: true }) // Atomic line with proper spacing
        
        // CRITICAL: Explicit spacing after raw hash block to prevent merge with "Generated by"
        currentY += 2
      } else {
      ...
      // Generated by (system name, not user email) - CRITICAL: sanitize and use writeLine for atomic separation
      const generatedByText = sanitizeText('Generated by: RiskMate Platform')
      currentY = writeLine(generatedByText, 8, STYLES.fonts.body, 11) // Use writeLine for atomic line
      
      // RLS/Org-scoped trust signal - CRITICAL: sanitize and use writeLine for atomic separation
      const orgScopedText = sanitizeText('Org-scoped: enforced')
      currentY = writeLine(orgScopedText, 8, STYLES.fonts.body, 11) // Use writeLine for atomic line
      ...
      await supabase
        ...
        .update({
          ...
          metadata: {
            ...
            pdf_hash: hash, // PDF file hash (SHA-256 of actual PDF bytes)
            metadata_hash: hash, // Metadata hash (deterministic - same as displayed in PDF)
          },
        })
        .eq('id', reportId)
    }
