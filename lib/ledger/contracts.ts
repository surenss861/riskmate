...
export interface LedgerEventContract {
  ...
  required_fields: {
    ...
    // Conditionally required based on event
 ...
 */
export const LEDGER_EVENT_CONTRACTS: Record<LedgerEventType, LedgerEventContract> = {
  // Review Queue Events
  'review.assigned': {
...
export function validateLedgerEvent(
  ...
  const contract = LEDGER_EVENT_CONTRACTS[eventType]
  if (!contract) {
 ...
 */
export function getEventContract(eventType: LedgerEventType): LedgerEventContract | null {
  return LEDGER_EVENT_CONTRACTS[eventType] || null
}

/**
 * Get all event types for a given category
 */
export function getEventsByCategory(category: LedgerCategory): LedgerEventType[] {
  return Object.entries(LEDGER_EVENT_CONTRACTS)
    .filter(([_, contract]) => contract.category === category)
    .map(([eventType]) => eventType as LedgerEventType)
}
