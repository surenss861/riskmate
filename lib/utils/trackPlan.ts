...
export async function trackPlanEvent(event: PlanTrackingEvent): Promise<void> {
  try {
    ...
    // Plans are organization-level, but we record which user (actor) initiated the action
    if (event.current_plan) {
      ...
      // Track in plan_tracking table (organization-level, but record who initiated)
      await supabase.from('plan_tracking').insert({
        organization_id: event.organization_id,
        actor_id: event.user_id, // User who initiated (plans are org-level)
