...
const supabaseClient_2 = require("../../lib/supabaseClient");
...
async function setupTestData() {
    ...
    const adminClient = (0, supabaseClient_2.getSupabaseAdmin)();
    ...
    const { error: ownerUserError } = await supabaseClient_1.supabase
        .from("users")
        .upsert({
        id: ownerAuthId,
        email: ownerEmail,
    ...
    const { error: executiveUserError } = await supabaseClient_1.supabase
        .from("users")
        .upsert({
        id: executiveAuthId,
    ...
    const { error: auditorUserError } = await supabaseClient_1.supabase
        .from("users")
        .upsert({
        id: auditorAuthId,
        email: auditorEmail,
    ...
    const anonClient = (0, supabase_js_1.createClient)(supabaseUrl, supabaseAnonKey);
    ...
    // Create a test job
    const { data: testJob, error: jobError } = await supabaseClient_1.supabase
        .from("jobs")
        .insert({
        organization_id: testOrgId,
 ...
 */
async function cleanupTestData(testOrgId) {
