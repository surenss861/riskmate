...
async function logTeamEvent(organizationId, actorId, eventType, eventName, targetUserId, targetInviteId, metadata = {}) {
    try {
        await supabaseClient_1.supabase.from("team_events").insert({
...
exports.teamRouter.get("/", async (req, res) => {
    ...
    try {
        ...
        let membersQuery = supabaseClient_1.supabase
            .from("users")
...
exports.teamRouter.post("/invite", (0, rbac_1.requireRole)("safety_lead"), async (req, res) => {
    ...
    try {
        ...
        // Log team event
        await logTeamEvent(organizationId, authReq.user.id, "invite_sent", "team.invite_sent", newUserId, inviteRow?.id || null, {
            email: normalizedEmail,
            role,
        });
...
exports.teamRouter.delete("/invite/:id", (0, rbac_1.requireRole)("admin"), async (req, res) => {
    ...
    try {
        ...
        // Log team event
        await logTeamEvent(authReq.user.organization_id, authReq.user.id, "invite_revoked", "team.invite_revoked", inviteRow?.user_id || null, authReq.params.id, {
...
exports.teamRouter.delete("/member/:id", (0, rbac_1.requireRole)("admin"), async (req, res) => {
    ...
    try {
        ...
        let targetMemberQuery = supabaseClient_1.supabase
            .from("users")
        ...
        if (targetMember.role === "owner") {
            ...
            let ownerCountQuery = supabaseClient_1.supabase
                .from("users")
                .select("id", { count: "exact", head: true })
                .eq("organization_id", authReq.user.organization_id)
                .eq("role", "owner");
        ...
        try {
            ...
            const { error: checkError } = await supabaseClient_1.supabase
                .from("users")
        ...
        // Log team event
        await logTeamEvent(authReq.user.organization_id, authReq.user.id, "access_revoked", "team.member_removed", authReq.params.id, null, {
...
exports.teamRouter.post("/acknowledge-reset", async (req, res) => {
    ...
    try {
        ...
        const { data: invite } = await supabaseClient_1.supabase
            .from("organization_invites")
        ...
        // Log team event
        if (invite) {
            await logTeamEvent(authReq.user.organization_id, authReq.user.id, "invite_accepted", "team.invite_accepted", authReq.user.id, invite.id, {
                email: invite.email,
                role: invite.role,
            });
        }
