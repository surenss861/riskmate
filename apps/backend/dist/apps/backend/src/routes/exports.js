...
exports.exportsRouter.post('/jobs/:id/export/pdf', auth_1.authenticate, rateLimiter_1.exportRateLimiter, async (req, res) => {
    ...
    try {
        ...
        // Check idempotency
        if (idempotencyKey) {
            const { data: existingExport } = await supabaseClient_1.supabase
                .from('exports')
                .select('*')
                .eq('organization_id', organization_id)
        ...
        // Create export job
        const { data: exportJob, error: insertError } = await supabaseClient_1.supabase
            .from('exports')
            .insert({
            organization_id,
            ...
            .select()
            .single();
        (0, structuredLog_1.logWithRequest)('info', 'Export job created', requestId, {
        ...
        res.setHeader('X-Error-ID', errorId);
        (0, errorResponse_1.logErrorForSupport)(500, 'EXPORT_REQUEST_ERROR', requestId, authReq.user?.organization_id, errorResponse.message, errorResponse.internal_message, 'operations', 'error', '/api/jobs/:id/export/pdf');
        res.status(500).json(errorResponse);
    }
});
// POST /api/jobs/:id/export/proof-pack
// Creates an async export job for a proof pack (ZIP)
exports.exportsRouter.post('/jobs/:id/export/proof-pack', auth_1.authenticate, rateLimiter_1.exportRateLimiter, async (req, res) => {
    const authReq = req;
    const requestId = authReq.requestId || 'unknown';
    try {
        const { organization_id, id: userId } = authReq.user;
...
// GET /api/exports/:id
// Poll export status
exports.exportsRouter.get('/exports/:id', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const requestId = authReq.requestId || 'unknown';
    try {
        const { organization_id } = authReq.user;
        ...
        res.setHeader('X-Error-ID', errorId);
        (0, errorResponse_1.logErrorForSupport)(500, 'EXPORT_FETCH_ERROR', requestId, authReq.user?.organization_id, errorResponse.message, errorResponse.internal_message, 'operations', 'error', '/api/exports/:id');
        res.status(500).json(errorResponse);
    }
});
// GET /api/exports/:id/download
// Stream export file
exports.exportsRouter.get('/exports/:id/download', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const requestId = authReq.requestId || 'unknown';
    try {
        const { organization_id } = authReq.user;
        ...
        if (downloadError || !fileData) {
            ...
            res.setHeader('X-Error-ID', errorId);
            (0, errorResponse_1.logErrorForSupport)(500, 'STORAGE_ERROR', requestId, organization_id, errorResponse.message, errorResponse.internal_message, 'operations', 'error', '/api/exports/:id/download');
            return res.status(500).json(errorResponse);
        }
        ...
        res.setHeader('X-Error-ID', errorId);
        (0, errorResponse_1.logErrorForSupport)(500, 'EXPORT_DOWNLOAD_ERROR', requestId, authReq.user?.organization_id, errorResponse.message, errorResponse.internal_message, 'operations', 'error', '/api/exports/:id/download');
        res.status(500).json(errorResponse);
    }
});
