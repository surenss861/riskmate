...
exports.invalidateExecutiveCache = invalidateExecutiveCache;
const express_1 = __importDefault(require("express"));
const supabaseClient_1 = require("../lib/supabaseClient");
const auth_1 = require("../middleware/auth");
const errorResponse_1 = require("../utils/errorResponse");
const executiveBrief_1 = require("../utils/pdf/executiveBrief");
const audit_1 = require("../middleware/audit");
const email_1 = require("../utils/email");
exports.executiveRouter = express_1.default.Router();
...
exports.executiveRouter.get('/risk-posture', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    ...
    try {
        ...
        const timeRange = authReq.query.time_range || '30d';
        ...
        let jobsQuery = supabaseClient_1.supabase
            .from('jobs')
        ...
        const openIncidents = (jobs || []).filter(j => j.status === 'incident' || (j.review_flag === true && j.risk_score !== null && j.risk_score > 75)).length;
        ...
        if (dateCutoff && timeRangeValue !== 'all') {
            ...
            const { data: prevJobs } = await supabaseClient_1.supabase
                .from('jobs')
            ...
            previousPeriodCounts.openIncidents = (prevJobs || []).filter(j => j.status === 'incident' || (j.review_flag === true && j.risk_score !== null && j.risk_score > 75)).length;
            // Count violations in previous period
            const { count: prevViolationsCount } = await supabaseClient_1.supabase
                .from('audit_logs')
            ...
            const { count: prevProofPacksCount } = await supabaseClient_1.supabase
                .from('audit_logs')
                .select('*', { count: 'exact', head: true })
                .eq('organization_id', organization_id)
            ...
            if (prevJobs && prevJobs.length > 0) {
                const { data: prevSignoffs } = await supabaseClient_1.supabase
                    .from('job_signoffs')
        ...
        if (jobs && jobs.length > 0) {
            const { data: signoffs } = await supabaseClient_1.supabase
                .from('job_signoffs')
                .select('job_id, status')
        ...
        let violationsQuery = supabaseClient_1.supabase
            .from('audit_logs')
            .select('*', { count: 'exact', head: true })
            .eq('organization_id', organization_id)
            .eq('event_type', 'auth.role_violation');
        ...
        let proofPacksQuery = supabaseClient_1.supabase
            .from('audit_logs')
            .select('*', { count: 'exact', head: true })
            .eq('organization_id', organization_id)
            .like('event_type', 'proof_pack.%');
        ...
        let lastMaterialEventQuery = supabaseClient_1.supabase
            .from('audit_logs')
            .select('created_at')
            .eq('organization_id', organization_id)
        ...
        const { data: allLogs, error: integrityError } = await supabaseClient_1.supabase
            .from('audit_logs')
            .select('id, hash, prev_hash, created_at')
            .eq('organization_id', organization_id)
            .order('created_at', { ascending: true });
        let ledgerIntegrity = 'not_verified';
        let lastVerifiedAt = null;
        let verifiedThroughEventId = null;
        let integrityErrorDetails = null;
        ...
        const { data: materialEvents } = await supabaseClient_1.supabase
            .from('audit_logs')
            .select('id')
            .eq('organization_id', organization_id)
            .in('severity', ['material', 'critical'])
            ...
            .limit(10);
        if (materialEvents) {
            basisEventIds.push(...materialEvents.map((e) => e.id));
        }
        const getTopDrivers = async () => {
            ...
            if (jobs && jobs.length > 0) {
                const { data: pendingSignoffs } = await supabaseClient_1.supabase
                    .from('job_signoffs')
                    .select('job_id, status')
                    .in('job_id', jobs.map(j => j.id))
                    .neq('status', 'signed');
        ...
        res.setHeader('X-Error-ID', errorId);
        (0, errorResponse_1.logErrorForSupport)(500, 'EXECUTIVE_POSTURE_FAILED', requestId, authReq.user?.organization_id, errorResponse.message, errorResponse.internal_message, errorResponse.category, errorResponse.severity, '/api/executive/risk-posture');
        res.status(500).json(errorResponse);
    }
});
// POST /api/executive/brief/pdf
// Generates PDF Board Brief from executive summary
exports.executiveRouter.post('/brief/pdf', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const requestId = authReq.requestId || 'unknown';
    try {
        ...
        const { data: org, error: orgError } = await supabaseClient_1.supabase
            .from('organizations')
            .select('name')
        ...
        const timeRange = req.body.time_range || '30d';
        ...
        // Generate PDF
        const { buffer, hash } = await (0, executiveBrief_1.generateExecutiveBriefPDF)(briefData, org.name || 'Organization', userEmail || `User ${userId.slice(0, 8)}`);
        // Record audit log
        ...
        res.setHeader('X-Error-ID', errorId);
        (0, errorResponse_1.logErrorForSupport)(500, 'PDF_GENERATION_FAILED', requestId, authReq.user?.organization_id, errorResponse.message, errorResponse.internal_message, errorResponse.category, errorResponse.severity, '/api/executive/brief/pdf');
        res.status(500).json(errorResponse);
    }
});
...
exports.executiveRouter.post('/alerts/check', async (req, res) => {
    const authReq = req;
    const requestId = authReq.requestId || 'unknown';
    try {
        ...
        if (cronSecret && expectedCronSecret && cronSecret === expectedCronSecret) {
            ...
            const { data: orgs, error: orgError } = await supabaseClient_1.supabase
                .from('organizations')
                .select('id');
            if (orgError) {
        ...
        const timeRange = req.body.time_range || '7d';
        const alertsSent = [];
        // Process each organization
        for (const organizationId of organizationIds) {
            // Get organization email recipients (owners, admins, and executives if that role exists)
            const { data: recipients, error: recipientError } = await supabaseClient_1.supabase
                .from('users')
                .select('email, role')
          ...
          `,
            ...
            for (const trigger of triggers) {
                ...
                const payloadHash = (0, email_1.hashAlertPayload)(alertPayload);
        ...
        res.setHeader('X-Error-ID', errorId);
        (0, errorResponse_1.logErrorForSupport)(500, 'ALERT_CHECK_FAILED', requestId, undefined, errorResponse.message, errorResponse.internal_message, errorResponse.category, errorResponse.severity, '/api/executive/alerts/check');
        res.status(500).json(errorResponse);
    }
});
// Invalidate cache on audit log write (called from audit middleware)
// Invalidates all time_range variants for the organization
function invalidateExecutiveCache(organizationId) {
