...
const logCursorMisuse = (organizationId, sortMode) => {
    ...
    try {
                ...
                try {
                    ...
                    if (sortField === 'created_at') {
                        ...
                        if (parts.length === 3) {
                            ...
                            if (!isNaN(cursorRiskNum) && cursorTimestamp && cursorId) {
                                if (sortDirection === 'desc') {
                                    fallbackQuery = fallbackQuery.or(`risk_score.lt.${cursorRiskNum},and(risk_score.eq.${cursorRiskNum},created_at.lt.${cursorTimestamp}),and(risk_score.eq.${cursorRiskNum},created_at.eq.${cursorTimestamp},id.lt.${cursorId})`);
                                }
                                else {
                                    fallbackQuery = fallbackQuery.or(`risk_score.gt.${cursorRiskNum},and(risk_score.eq.${cursorRiskNum},created_at.gt.${cursorTimestamp}),and(risk_score.eq.${cursorRiskNum},created_at.eq.${cursorTimestamp},id.gt.${cursorId})`);
                                }
                            }
                        }
                    }
        ...
        if (jobIds.length > 0 && (useReadinessOrdering || useBlockersOrdering || !useCursor)) {
            ...
            const { data: mitigationItems } = await supabaseClient_1.supabase
                ...
                .in('job_id', jobIds);
            const { data: documents } = await supabaseClient_1.supabase
                .from('documents')
                .select('job_id')
                .in('job_id', jobIds);
            ...
            jobIds.forEach((jobId) => {
                ...
                let readiness_basis = 'not_calculated';
                let readiness_empty_reason = null;
        ...
        if (countError && (countError.message?.includes('archived_at') || countError.message?.includes('deleted_at') || countError.code === 'PGRST116')) {
            let fallbackCountQuery = supabaseClient_1.supabase
                .from("jobs")
                .select("*", { count: "exact", head: true })
                .eq("organization_id", organization_id);
            if (statusForQuery) {
...
function buildBulkResults(succeeded, failed) {
    const succeededResults = succeeded.map((job_id) => ({ job_id, status: "success" }));
    const failedResults = failed.map(({ id, code, message }) => ({
        job_id: id,
        status: "error",
        error: message,
        code,
...
exports.jobsRouter.post("/bulk/status", auth_1.authenticate, requireWriteAccess_1.requireWriteAccess, async (req, res) => {
    ...
    try {
        ...
        const validIds = job_ids.filter((id) => typeof id === "string");
        const failed = job_ids
        ...
        const found = new Map((jobs ?? []).map((j) => [j.id, j]));
        ...
        const { data: rpcResult, error: rpcError } = await supabaseClient_1.supabase.rpc("bulk_update_job_status", {
        ...
        const succeeded = Array.isArray(rpcResult)
            ? rpcResult.map((r) => (typeof r === "string" ? r : r?.id ?? "")).filter(Boolean)
            : [];
        for (const id of eligibleIds) {
...
exports.jobsRouter.post("/bulk/assign", auth_1.authenticate, requireWriteAccess_1.requireWriteAccess, async (req, res) => {
    const authReq = req;
    try {
        ...
        const { data: assignee, error: assigneeError } = await supabaseClient_1.supabase
            .from("users")
        ...
        const validIds = jobIdsRaw.filter((id) => typeof id === "string");
        const failed = jobIdsRaw
            .filter((id) => typeof id !== "string")
            .map((id) => ({ id: String(id), code: "INVALID_ID", message: "Invalid job id" }));
        if (validIds.length === 0) {
            const total = failed.length;
        ...
        const { data: rpcResult, error: rpcError } = await supabaseClient_1.supabase.rpc("bulk_assign_jobs", {
            p_organization_id: organization_id,
            p_job_ids: eligibleIds,
        ...
        for (const jobId of succeeded) {
            ...
            try {
                await (0, notifications_1.sendJobAssignedNotification)(workerId, organization_id, jobId, job?.client_name ?? undefined);
            }
            catch (_) { }
            await (0, audit_1.recordAuditLog)({
                organizationId: organization_id,
...
exports.jobsRouter.post("/bulk/delete", auth_1.authenticate, requireWriteAccess_1.requireWriteAccess, async (req, res) => {
    ...
    try {
        ...
        if (!(0, permissions_1.hasJobsDeletePermission)(role)) {
        ...
        const [auditByTarget, auditByJobId, riskRes, reportRes, reportRunsRes,] = await Promise.all([
            ...
            supabaseClient_1.supabase
                ...
                .not("job_id", "is", null),
            supabaseClient_1.supabase.from("job_risk_scores").select("job_id").in("job_id", draftNotDeleted),
            supabaseClient_1.supabase.from("reports").select("job_id").in("job_id", draftNotDeleted),
            supabaseClient_1.supabase
                .from("report_runs")
                .select("job_id")
                .eq("organization_id", organization_id)
        ...
        const hasAuditByTarget = new Set((auditByTarget.data ?? []).map((r) => r.target_id));
        const hasAuditByJobId = new Set((auditByJobId.data ?? []).map((r) => r.job_id).filter(Boolean));
        const hasRisk = new Set((riskRes.data ?? []).map((r) => r.job_id));
        const hasReports = new Set([
            ...(reportRes.data ?? []).map((r) => r.job_id),
            ...(reportRunsRes.data ?? []).map((r) => r.job_id),
        ]);
        ...
        const { error: rpcError } = await supabaseClient_1.supabase.rpc("bulk_soft_delete_jobs", {
            p_organization_id: organization_id,
            p_job_ids: eligibleIds,
        ...
        if (rpcError) {
            ...
            const pgCode = rpcError.code;
            const isEligibilityError = pgCode === "P0001" ||
                pgCode === "23514" ||
        ...
        const auditPromises = eligibleIds.map((jobId) => {
            ...
            return (0, audit_1.recordAuditLog)({
                organizationId: organization_id,
                actorId: userId,
...
exports.jobsRouter.post("/bulk/export", auth_1.authenticate, async (req, res) => {
    const authReq = req;
    try {
        const { organization_id } = authReq.user;
        const { job_ids } = req.body || {};
...
exports.jobsRouter.patch("/:id/flag", auth_1.authenticate, requireWriteAccess_1.requireWriteAccess, async (req, res) => {
    ...
    try {
        ...
        if (role === 'member' || role === 'executive') {
            ...
            res.setHeader('X-Error-ID', errorId);
            (0, errorResponse_1.logErrorForSupport)(403, "AUTH_ROLE_FORBIDDEN", requestId, organization_id, errorResponse.message, errorResponse.internal_message, errorResponse.category, errorResponse.severity, '/api/jobs/:id/flag');
        ...
        const { data: updatedJob, error: updateError } = await supabaseClient_1.supabase
            .from("jobs")
            .update(updateData)
            .eq("id", id)
            .eq("organization_id", organization_id)
        ...
        if (updateError) {
            ...
            res.setHeader('X-Error-ID', errorId);
            (0, errorResponse_1.logErrorForSupport)(500, "JOB_FLAG_UPDATE_FAILED", requestId, organization_id, errorResponse.message, errorResponse.internal_message, errorResponse.category, errorResponse.severity, '/api/jobs/:id/flag');
            return res.status(500).json(errorResponse);
        }
        // Log audit event with comprehensive review context
        try {
            const { data: jobData } = await supabaseClient_1.supabase
                .from("jobs")
        ...
        res.setHeader('X-Error-ID', errorId);
        (0, errorResponse_1.logErrorForSupport)(500, "JOB_FLAG_ERROR", authReq.requestId || 'unknown', authReq.user?.organization_id, errorResponse.message, errorResponse.internal_message, errorResponse.category, errorResponse.severity, '/api/jobs/:id/flag');
        res.status(500).json(errorResponse);
    }
});
// DELETE /api/jobs/:id
...
exports.jobsRouter.delete("/:id", auth_1.authenticate, requireWriteAccess_1.requireWriteAccess, async (req, res) => {
    ...
    try {
        ...
        if (!(0, permissions_1.canDeleteSingleJob)(role)) {
            const requestId = authReq.requestId || 'unknown';
            const { response: errorResponse, errorId } = (0, errorResponse_1.createErrorResponse)({
            ...
            res.setHeader('X-Error-ID', errorId);
            (0, errorResponse_1.logErrorForSupport)(403, "AUTH_ROLE_FORBIDDEN", requestId, organization_id, errorResponse.message, errorResponse.internal_message, errorResponse.category, errorResponse.severity, '/api/jobs/:id');
            return res.status(403).json(errorResponse);
        }
        // Get job and verify ownership
        const { data: job, error: jobError } = await supabaseClient_1.supabase
...
exports.jobsRouter.post("/:id/proof-pack", auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const requestId = authReq.requestId || 'unknown';
    try {
        const { organization_id, id: userId } = authReq.user;
...
exports.jobsRouter.get("/:id/signoffs", auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const requestId = authReq.requestId || 'unknown';
    try {
        const { organization_id } = authReq.user;
...
exports.jobsRouter.post("/:id/signoffs", auth_1.authenticate, requireWriteAccess_1.requireWriteAccess, async (req, res) => {
    ...
    try {
        ...
        const { data: memberData } = await supabaseClient_1.supabase
