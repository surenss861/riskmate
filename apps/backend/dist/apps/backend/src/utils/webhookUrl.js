...
async function resolveAndCheckAddresses(hostname) {
    ...
    try {
        const [v4, v6] = await Promise.all([
            node_dns_1.promises.resolve4(hostname).catch(() => []),
            node_dns_1.promises.resolve6(hostname).catch(() => []),
        ]);
...
function isLiteralIpv6(host) {
    return (0, node_net_1.isIP)(host) === 6;
}
 ...
 */
function getIpv4MappedDottedDecimal(host) {
    const normalized = host.toLowerCase().trim();
    ...
    if (!groups)
        return null;
    const isStandardMapped = groups[0] === 0 &&
        groups[1] === 0 &&
        groups[2] === 0 &&
        groups[3] === 0 &&
        groups[4] === 0 &&
        groups[5] === 0xffff;
    const isAlternateMapped = groups[0] === 0 &&
        groups[1] === 0 &&
        groups[2] === 0 &&
        groups[3] === 0 &&
        groups[4] === 0xffff &&
        groups[5] === 0;
    if (!isStandardMapped && !isAlternateMapped)
        return null;
    const hi = groups[6];
    const lo = groups[7];
    const addr = (hi << 16) | lo;
