...
exports.mapCategoryToTab = mapCategoryToTab;
exports.eventBelongsToTab = eventBelongsToTab;
/**
 * Map any category (including old/legacy values) to the main Compliance Ledger tab
 */
function mapCategoryToTab(category, eventName) {
    if (!category) {
 ...
 */
function computeCategoryFromEventName(eventName) {
    const name = eventName.toLowerCase();
    // Governance Enforcement: blocked actions, policy enforcement, violations
    if (name.includes('auth.role_violation') ||
 ...
 */
function eventBelongsToTab(category, eventName, tab) {
