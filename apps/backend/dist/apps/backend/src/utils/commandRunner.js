...
exports.FilterValidationError = exports.VALID_SAVED_VIEWS = exports.VALID_TIME_RANGES = exports.VALID_AUDIT_CATEGORIES = void 0;
exports.runCommand = runCommand;
exports.applyAuditFilters = applyAuditFilters;
 ...
 */
async function runCommand(supabase, ctx, options, commandFn, auditEntry) {
    const { idempotencyKey, skipLedger = false } = options;
    const requestId = ctx.requestId || (0, uuid_1.v4)();
    try {
        ...
        if (idempotencyKey) {
            ...
            if (existing) {
                ...
                return {
                    ok: true,
                    data: existing.metadata?.command_result,
        ...
        if (!skipLedger) {
            const ledgerResult = await (0, audit_1.recordAuditLog)({
...
exports.VALID_AUDIT_CATEGORIES = [
    'governance',
    'operations',
    'access',
    'review_queue',
...
exports.VALID_TIME_RANGES = ['24h', '7d', '30d', 'all', 'custom'];
exports.VALID_SAVED_VIEWS = ['review-queue', 'insurance-ready', 'governance-enforcement', 'incident-review', 'access-review'];
/**
 * Validation error for invalid filter parameters
 */
class FilterValidationError extends Error {
    constructor(message, field, allowedValues) {
        super(message);
        this.field = field;
...
function applyAuditFilters(query, filters) {
    ...
    // Validate category
    if (category && !exports.VALID_AUDIT_CATEGORIES.includes(category)) {
        throw new FilterValidationError(`Invalid category: ${category}`, 'category', exports.VALID_AUDIT_CATEGORIES);
    }
    // Validate time_range
    if (time_range && !exports.VALID_TIME_RANGES.includes(time_range)) {
        throw new FilterValidationError(`Invalid time_range: ${time_range}`, 'time_range', exports.VALID_TIME_RANGES);
    }
    // Validate view
    if (view && !exports.VALID_SAVED_VIEWS.includes(view)) {
        throw new FilterValidationError(`Invalid view: ${view}`, 'view', exports.VALID_SAVED_VIEWS);
    }
    // Validate custom time range requires dates
    if (time_range === 'custom' && (!start_date || !end_date)) {
        throw new FilterValidationError('Custom time_range requires both start_date and end_date', 'time_range');
    }
    ...
    if (view) {
        ...
        if (view === 'review-queue') {
            ...
            query = query.eq('category', 'operations').in('event_name', ['job.completed', 'control.verified', 'evidence.uploaded', 'attestation.created']);
        }
