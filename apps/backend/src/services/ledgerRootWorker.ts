...
let rootWorkerInterval: NodeJS.Timeout | null = null

/**
 * Start the ledger root worker
 */
export function startLedgerRootWorker() {
 ...
 */
export function stopLedgerRootWorker() {
...
async function computeDailyRoots() {
  try {
    ...
    logStructured('info', 'Computing daily ledger roots', {
      date: dateStr,
    })
    ...
    logStructured('info', 'Daily ledger roots computed', {
      date: dateStr,
      org_count: orgs.length,
    })
  } catch (err: any) {
    logStructured('error', 'Failed to compute daily ledger roots', {
      error: err.message,
    })
  }
}
...
async function computeOrgRoot(
  ...
  try {
    ...
    logStructured('info', 'Computed ledger root for org', {
    ...
    logStructured('error', 'Failed to compute org ledger root', {
      org_id: organizationId,
      date: dateStr,
      error: err.message,
    })
