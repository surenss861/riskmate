...
function runProductionStartupEnvChecks(): void {
  ...
  const webhookDisabled =
    process.env.DISABLE_WEBHOOK_WORKER === "true" || process.env.DISABLE_WEBHOOK_WORKER === "1";
...
app.get("/health", (_req, res) => {
  ...
  if (webhookWorkerDegraded) body.webhook_worker = "degraded";
  res.json(body);
});
  ...
  const server = app.listen(PORT, '0.0.0.0', async () => {
    ...
    if (process.env.SKIP_WORKERS !== 'true') {
      ...
      if (!disableWebhookWorker) {
        const workerResult = await startWebhookDeliveryWorker();
