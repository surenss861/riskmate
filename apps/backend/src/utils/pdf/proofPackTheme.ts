...
export function drawHeader(doc: PDFKit.PDFDocument, options: HeaderOptions): void {
  ...
  // Title with orange accent underline
  doc
    .fillColor(STYLES.colors.primaryText)
    .fontSize(STYLES.sizes.h1)
    .font(STYLES.fonts.header)
    .text(options.title, margin, margin, { align: 'left' })
  ...
  if (options.packId) {
    doc.text(`Pack ID: ${options.packId}`, { align: 'left' })
  }
  if (options.organizationName) {
    doc.text(`Organization: ${options.organizationName}`, { align: 'left' })
  }
  if (options.generatedBy && options.generatedByRole) {
    doc.text(`Generated By: ${options.generatedBy} (${options.generatedByRole})`, { align: 'left' })
  }
  if (options.generatedAt) {
    doc.text(`Generated: ${new Date(options.generatedAt).toLocaleString()}`, { align: 'left' })
  }
  if (options.timeRange) {
    doc.text(`Time Range: ${options.timeRange}`, { align: 'left' })
  }
...
export function drawKpiRow(doc: PDFKit.PDFDocument, stats: KPIRow[]): void {
  ...
  stats.forEach((stat, index) => {
    ...
    doc
      ...
      .fontSize(STYLES.sizes.caption)
      .font(STYLES.fonts.body)
      .text(stat.label, x + 8, startY + 6, { width: tileWidth - 16, align: 'left' })
    ...
    doc
      ...
      .font(STYLES.fonts.header)
      .text(String(stat.value), x + 8, startY + 20, { width: tileWidth - 16, align: 'left' })
  })
...
export function drawTable(
  ...
  const drawTableRow = (row: any[], y: number, isEven: boolean) => {
    ...
    normalizedColumns.forEach((col, colIndex) => {
      ...
      const text = String(cellValue).substring(0, 100) // Truncate long values
...
export function drawEmptyState(
  ...
  doc
    ...
    .fontSize(STYLES.sizes.h3)
    .font(STYLES.fonts.header)
    .text(options.title, margin + 16, cardY + 16, { align: 'left' })
  ...
  doc
    ...
    .fontSize(STYLES.sizes.body)
    .font(STYLES.fonts.body)
    .text(options.message, margin + 16, cardY + 40, {
      width: cardWidth - 32,
      align: 'left',
    })
  ...
  if (options.filters && Object.keys(options.filters).length > 0) {
    ...
    if (filterText) {
      doc
        .fontSize(STYLES.sizes.caption)
        .text(`Applied filters: ${filterText}`, margin + 16, cardY + 70, {
          width: cardWidth - 32,
          align: 'left',
        })
    }
  ...
  if (options.actionHint) {
    doc
      ...
      .fontSize(STYLES.sizes.caption)
      .font(STYLES.fonts.body)
      .text(options.actionHint, margin + 16, cardY + 90, {
        width: cardWidth - 32,
        align: 'left',
      })
  }
