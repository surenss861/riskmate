/** Assignee shape used in API responses (matches frontend types/tasks.ts). */
 ...
 */
export function mapTaskToApiShape<T extends TaskRowWithAssignee>(
  row: T
): Omit<T, "assignee"> & { assigned_user: TaskAssignedUser | null } {
  ...
  const assigned_user: TaskAssignedUser | null =
    assignee && typeof assignee === "object" && "id" in assignee
      ...
      : null;
  return { ...rest, assigned_user } as Omit<T, "assignee"> & {
