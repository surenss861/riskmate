...
export const WEBHOOK_SECRET_DECRYPTION_ERROR =
  'WEBHOOK_SECRET_ENCRYPTION_KEY is missing or invalid; ensure web and backend use the same key'
...
export function decryptWebhookSecret(ciphertext: string, keyHex: string | undefined): string {
  ...
  if (!keyHex || typeof keyHex !== 'string') {
    throw new Error(WEBHOOK_SECRET_DECRYPTION_ERROR)
  }
  const trimmed = keyHex.trim()
  if (!trimmed) {
    throw new Error(WEBHOOK_SECRET_DECRYPTION_ERROR)
