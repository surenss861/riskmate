...
export const publicVerificationRouter: ExpressRouter = express.Router()
...
publicVerificationRouter.get(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    try {
      ...
      const { data: ledgerRoot } = await supabase
        .from('ledger_roots')
        .select('*')
      ...
      const ledgerMatch = ledgerEvent && 
        (ledgerEvent as any).metadata?.manifest_hash === exportJob.manifest_hash
