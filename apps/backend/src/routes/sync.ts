...
syncRouter.post(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    try {
      const { organization_id, id: userId } = authReq.user;
      const { operation_id, strategy, resolved_value, entity_type, entity_id, operation_type } = req.body as {
        operation_id: string;
      ...
      let updatedMitigationItem: any = null;
      ...
      if (strategy === "server_wins") {
        ...
        if (opType === "update_job") {
          ...
          if (opType === "delete_hazard") {
            const { error } = await supabase.rpc("sync_delete_hazard", {
              p_organization_id: organization_id,
              p_job_id: jobId,
