  ...
  async (req: express.Request, res: express.Response) => {
    ...
    try {
      ...
      const [
        currentSummaryRes,
        previousSummaryRes,
        currentComplianceRes,
        previousComplianceRes,
        jobsAtRiskRes,
        missingEvidenceRes,
        chartDataRes,
      ] = await Promise.all([
        supabase.rpc("get_dashboard_summary_kpis", {
        ...
        supabase.rpc("get_dashboard_jobs_at_risk", {
          p_org_id: organization_id,
          p_since: currentSince,
          p_until: currentUntil,
          p_limit: 10,
        }),
        supabase.rpc("get_dashboard_missing_evidence_jobs", {
          p_org_id: organization_id,
          p_since: currentSince,
          p_until: currentUntil,
          p_limit: 10,
      ...
      if (currentSummaryRes.error) throw currentSummaryRes.error;
      if (previousSummaryRes.error) throw previousSummaryRes.error;
      if (currentComplianceRes.error) throw currentComplianceRes.error;
      if (previousComplianceRes.error) throw previousComplianceRes.error;
      if (jobsAtRiskRes.error) throw jobsAtRiskRes.error;
      if (missingEvidenceRes.error) throw missingEvidenceRes.error;
      if (chartDataRes.error) throw chartDataRes.error;
      ...
      const jobsAtRisk = ((jobsAtRiskRes.data ?? []) as { id: string; client_name: string | null; job_type: string | null; location: string | null; status: string | null; risk_score: number | null; risk_level: string | null; created_at: string }[]).map((job) => ({
        id: job.id,
      ...
      const missingEvidenceJobs = ((missingEvidenceRes.data ?? []) as { id: string; client_name: string | null; job_type: string | null; location: string | null; status: string | null; risk_score: number | null; risk_level: string | null; created_at: string }[]).map((job) => ({
        id: job.id,
        client_name: job.client_name ?? undefined,
        job_type: job.job_type ?? undefined,
        location: job.location ?? undefined,
      ...
      const chartRows = (chartDataRes.data ?? []) as { period_key: string; jobs_created: number; jobs_completed: number }[];
      const chartByDate = new Map<string, { jobs_created: number; jobs_completed: number }>();
      ...
      for (let i = chartDays - 1; i >= 0; i--) {
        ...
        const value = cur.jobs_created === 0 ? 0 : Math.round((cur.jobs_completed / cur.jobs_created) * 100);
