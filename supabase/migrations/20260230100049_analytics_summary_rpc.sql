-- Analytics summary: single RPC returning job_counts_by_status, risk_level_distribution, evidence stats, top team activity.
-- Replaces client-side pagination over jobs/documents/mitigation_items to meet <500ms SLA.
...
AS $$
  WITH period_jobs AS (
    SELECT j.id, j.status, j.risk_level
    FROM jobs j
...
COMMENT ON FUNCTION get_analytics_summary(UUID, TIMESTAMPTZ, TIMESTAMPTZ) IS
  'Analytics summary in one call: job_counts_by_status, risk_level_distribution, evidence stats, top 20 team activity; filtered to [p_since, p_until].';
