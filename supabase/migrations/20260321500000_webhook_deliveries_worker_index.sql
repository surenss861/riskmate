...
-- Runs after 20260320 (terminal_outcome column) so the predicate is valid.
CREATE INDEX IF NOT EXISTS idx_webhook_deliveries_worker ON webhook_deliveries(next_retry_at, attempt_count)
  WHERE delivered_at IS NULL AND processing_since IS NULL AND next_retry_at IS NOT NULL AND terminal_outcome IS NULL;
