-- Add jobs_completed metric to day-level trends: real completed counts by completion date (no rate).
-- Enables Jobs-over-time chart to show actual completed counts and correct drilldown by completion date.
...
BEGIN
  ...
  -- Completion: one row per day in range; value = completion rate 0–100.
  IF p_metric = 'completion' THEN
    ...
    completed AS (
      SELECT (j.completed_at AT TIME ZONE 'UTC')::DATE AS d, COUNT(*)::BIGINT AS c
      FROM jobs j
      WHERE j.organization_id = p_org_id
        AND j.deleted_at IS NULL
      ...
      GROUP BY (j.completed_at AT TIME ZONE 'UTC')::DATE
    )
    SELECT
      days.period_key,
      (CASE
  ...
  -- jobs_completed: one row per day; value = count of jobs completed on that day (by completed_at). For chart completed series and drilldown by completion date.
  IF p_metric = 'jobs_completed' THEN
    RETURN QUERY
    WITH days AS (
      SELECT generate_series(
        (p_since AT TIME ZONE 'UTC')::DATE,
    ...
    SELECT
      days.period_key,
      COALESCE(completed.c, 0)::NUMERIC AS value
    FROM days
    LEFT JOIN completed ON completed.d = days.period_key
    ORDER BY days.period_key;
    RETURN;
...
COMMENT ON FUNCTION get_trends_day_buckets(UUID, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) IS
  'Day-level trend buckets: period_key (date), value. p_metric: jobs, risk, completion (rate 0–100), jobs_completed (count by completed_at), compliance.';
