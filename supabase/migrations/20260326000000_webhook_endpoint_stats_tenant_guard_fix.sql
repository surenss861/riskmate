...
AS $$
  ...
  WHERE we.organization_id = p_org_id
    AND (current_user = 'service_role' OR p_org_id IN (SELECT public.webhook_admin_org_ids()))
  GROUP BY wd.endpoint_id, ast.last_attempt_at, ast.last_failure_at;
$$;

COMMENT ON FUNCTION get_webhook_endpoint_stats(uuid) IS
  'Returns per-endpoint delivery counts and last delivery time for org. In-function tenant guard: service_role or p_org_id in webhook_admin_org_ids(). Only service_role may execute; API uses admin client.';
