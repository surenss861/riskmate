-- Exclude intentional cancellations (cancelled_paused, cancelled_policy) from failed count
-- and last_terminal_failure_at so paused endpoints do not show as failing.
...
AS $$
  ...
  SELECT
    ...
    max(wd.created_at) FILTER (WHERE wd.delivered_at IS NULL AND wd.next_retry_at IS NULL AND (wd.terminal_outcome IS NULL OR wd.terminal_outcome = 'failed')) AS last_terminal_failure_at,
    ast.last_failure_at
...
COMMENT ON FUNCTION get_webhook_endpoint_stats(uuid) IS
  'Returns per-endpoint delivery counts and last delivery time for org. failed and last_terminal_failure_at exclude intentional cancellations (cancelled_paused, cancelled_policy).';
