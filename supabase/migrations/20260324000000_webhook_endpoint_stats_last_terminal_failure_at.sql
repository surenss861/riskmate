-- Use last attempt time for last_terminal_failure_at instead of delivery created_at,
-- so the timestamp reflects when the delivery actually failed (last attempt), not when it was enqueued.
...
COMMENT ON FUNCTION get_webhook_endpoint_stats(uuid) IS
  'Returns per-endpoint delivery counts and last delivery time for org. failed and last_terminal_failure_at exclude intentional cancellations (cancelled_paused, cancelled_policy). last_terminal_failure_at uses the timestamp of the last attempt for terminal failures.';
