-- Exclude in-progress deliveries (processing_since IS NOT NULL) from the failed count.
-- When the worker claims a delivery it sets processing_since and clears next_retry_at, so without
-- this filter those rows would be counted as failed and briefly inflate the dashboard failure count.
...
COMMENT ON FUNCTION get_webhook_endpoint_stats(uuid) IS
  'Returns per-endpoint delivery counts and last delivery time for org. Excludes in-progress (processing_since set) from failed count. In-function tenant guard: session_user = service_role (caller-aware) or p_org_id in webhook_admin_org_ids(). Only service_role may execute; API uses admin client.';
